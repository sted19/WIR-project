...
if __name__ == "__main__":
    ...
    print('\n')
    print(rating_book.groupby('bookTitle').ISBN.nunique().sort_values(ascending=False)[:10])
    print('\n')
    multiple_isbns = rating_book.groupby('bookTitle').ISBN.nunique()
    ...
    #dict_unique_isbn = make_isbn_dict(rating_book)
    ...
    def add_unique_isbn_col(df):
        df['uniqueISBN'] = df.apply(lambda row: multiple_isbn_dict[row.bookTitle][0] if row.bookTitle in multiple_isbn_dict.keys() else row.ISBN, axis=1)
        return df

    rating_book = add_unique_isbn_col(rating_book)
